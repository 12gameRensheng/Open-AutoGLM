...
class PhoneAgent:
    ...
    RECOVERY_ACTIONS = [
        ("swipe_down", lambda h: do(action="Swipe", startPoint=[540, int(h*0.7)], endPoint=[540, int(h*0.3)], duration="500ms")),
        ("swipe_up", lambda h: do(action="Swipe", startPoint=[540, int(h*0.3)], endPoint=[540, int(h*0.7)], duration="500ms")),
        ("back", lambda h: do(action="Back")),
        ("wait", lambda h: do(action="Wait", duration="3 seconds")),
    ]
    ...
    def _step(self, user_prompt: str | None = None, is_first: bool = False) -> StepResult:
        ...
        self._log(f"Screen: {screenshot.width}x{screenshot.height} | App: {current_app}")
        ...
        # 6. Display thinking and action
        self._display_response(response.thinking, action)

        # 7. Remove image from context
        self._context[-1] = MessageBuilder.remove_images_from_message(self._context[-1])

        # 8. Execute action
        result = self.action_handler.execute(action, screenshot.width, screenshot.height)
        self._log(f"Result: success={result.success}")
